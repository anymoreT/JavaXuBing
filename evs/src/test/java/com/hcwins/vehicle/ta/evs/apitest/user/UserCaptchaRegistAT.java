...
import com.hcwins.vehicle.ta.evs.apiobj.user.CaptchaRegistResponse;
import com.hcwins.vehicle.ta.evs.apiobj.user.Login;
 ...
 */
public class UserCaptchaRegistAT extends EVSTestBase {
    static final Logger logger = LoggerFactory.getLogger(UserCaptchaRegistAT.class);
    private SubscriberData subscriberData0;
    ...
    @Test(description = "个人获取验证码接口测试:验证成功获取验证码")
    public void testCaptchaRegistSuccess() {
        ...
        CaptchaRegistResponse captchaRegistResponse = CaptchaRegist.postCaptchaRegistRequest(mobile10);
        assertThat(captchaRegistResponse.getResult().getCode(), equalTo(0));

        List<EVSCaptcha> captchas = CaptchaRegist.getCaptchaRegists(mobile10);
    ...
    @Test(description = "个人获取验证码接口测试:验证连续3次获取验证码，只有最后一次的有效，其余的为过期")
    public void testCaptchaRegistExistentNewBeSetToExpiredAfterGettingNewOne() {
        ...
        for (int i = 0; i < 3; i++) {
            captchas = CaptchaRegist.postAndGetCaptchas(mobile10);
            assertThat(captchas.size(), equalTo(1));
            if (i == 0) {
    ...
    @Test(description = "个人获取验证码接口测试:验证获取验证码时的数据更新mobile、module隔离")
    public void testCaptchaRegistExistentNewBeSetToExpiredAfterGettingNewOneIsMobileAndModuleIsolated() {
        ...
        captcha1.setModule(EVSCaptcha.Module.ENTEPRISE_REGISTER);
        assertThat(EVSCaptcha.dao.updateModule(captcha1), equalTo(1));
        captcha1 = EVSCaptcha.dao.findById(captcha1.getId());
        assertThat(captcha1.getModule(), equalTo(EVSCaptcha.Module.ENTEPRISE_REGISTER));

        EVSCaptcha captcha2 = CaptchaRegist.postAndGetCaptchas(mobile10).get(0);
        ...
        EVSCaptcha captcha3 = CaptchaRegist.postAndGetCaptchas(mobile10).get(0);
    ...
    @Test(description = "个人获取验证码接口测试:验证获取验证码的ErrorCode",
